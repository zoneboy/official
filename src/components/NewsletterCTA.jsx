...
export default function NewsletterCTA() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error("Failed"); setStatus("success"); setEmail("");
    } catch (error) { console.error(error); setStatus("idle"); alert("We encountered an issue. Please try again later."); }
  };
  return (
          ...
          <div style={{ position: "relative", zIndex: 10, maxWidth: 560 }}>
            ...
            {status === "success" ? (
              ...
              <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: isMobile ? "column" : "row", gap: 12 }}>
                <input type="email" required placeholder="Your work email" value={email} onChange={(e) => setEmail(e.target.value)} disabled={status === "loading"} style={{ flex: 1, padding: "14px 20px", borderRadius: 12, border: "none", fontSize: 14, outline: "none", opacity: status === "loading" ? 0.7 : 1 }} />
                <button type="submit" disabled={status === "loading"} style={{ background: "#fff", color: COLORS.primary, padding: "14px 28px", borderRadius: 12, border: "none", fontFamily: FONTS.headline, fontWeight: 700, cursor: status === "loading" ? "not-allowed" : "pointer", opacity: status === "loading" ? 0.8 : 1, minWidth: 120 }}>{status === "loading" ? "Subscribing..." : "Subscribe"}</button>
              </form>
            )}
          </div>
        </div>
