...
// The question is displayed; the token must be sent back on submit unchanged.
...
export default function NewsletterCTA({ compact }) {
  ...
  const loadNewChallenge = useCallback(async () => {
    ...
    try {
      ...
      setError(e.message || "Could not load verification. Please try again.");
      setChallenge(null);
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!challenge) { setError("Verification not ready. Try refreshing."); return; }
    ...
    try {
      const response = await fetch("/api/subscribe", {
        ...
        body: JSON.stringify({
          ...
          captcha_answer: parseInt(mathInput, 10),
        }),
      });
      ...
      setStatus("idle");
      // Load a new challenge after any failed attempt — the old token may be spent/expired.
      await loadNewChallenge();
    }
  };

  const questionText = loadingChallenge ? "Loading…" : (challenge?.question ? `${challenge.question} = ?` : "— = ?");
                      ...
                      <div style={{ background: "rgba(255,255,255,0.1)", border: "1px solid rgba(255,255,255,0.2)", borderRadius: 12, padding: isMobile ? "16px" : "18px 22px", marginBottom: 4 }}>
                        ...
                        <div style={{ display: "flex", flexDirection: isMobile ? "column" : "row", alignItems: isMobile ? "stretch" : "center", gap: 12 }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 10, flex: 1 }}>
                            <span style={{ fontFamily: "monospace", fontSize: 20, fontWeight: 800, color: "#fff", background: "rgba(0,0,0,0.25)", padding: "8px 16px", borderRadius: 8, letterSpacing: 3, whiteSpace: "nowrap" }}>{questionText}</span>
                            <input type="number" value={mathInput} onChange={(e) => { setMathInput(e.target.value); setError(""); }} onKeyDown={(e) => { if (e.key === "Enter") handleSubmit(); }} placeholder="?" disabled={loadingChallenge} style={{ width: 70, padding: "10px 14px", borderRadius: 8, border: "none", fontSize: 16, fontWeight: 700, textAlign: "center", outline: "none" }} />
                          </div>
                          <div style={{ display: "flex", gap: 8 }}>
                            <button type="button" onClick={handleSubmit} disabled={status === "loading" || loadingChallenge} style={{ background: "#fff", color: COLORS.primary, padding: "12px 24px", borderRadius: 10, border: "none", fontFamily: FONTS.headline, fontWeight: 700, cursor: (status === "loading" || loadingChallenge) ? "not-allowed" : "pointer", fontSize: 13, opacity: (status === "loading" || loadingChallenge) ? 0.7 : 1 }}>{status === "loading" ? "Subscribing..." : "Subscribe"}</button>
                            <button type="button" onClick={() => { setShowCaptcha(false); setMathInput(""); setError(""); setChallenge(null); }} style={{ background: "rgba(255,255,255,0.1)", color: "rgba(255,255,255,0.7)", padding: "12px 16px", borderRadius: 10, border: "1px solid rgba(255,255,255,0.15)", cursor: "pointer", fontSize: 13, fontWeight: 600 }}>Back</button>
                          </div>
                        </div>
                      </div>
                    </FadeIn>
          ...
          <div>
            <div style={{ background: "rgba(0,0,0,0.2)", border: "1px solid rgba(255,255,255,0.15)", borderRadius: 8, padding: 14, marginBottom: 10 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ fontFamily: "monospace", fontSize: 16, fontWeight: 800, color: "#fff", background: "rgba(0,0,0,0.3)", padding: "6px 12px", borderRadius: 6, letterSpacing: 2, whiteSpace: "nowrap" }}>{questionText}</span>
                <input type="number" value={mathInput} onChange={(e) => { setMathInput(e.target.value); setError(""); }} onKeyDown={(e) => { if (e.key === "Enter") handleSubmit(); }} placeholder="?" disabled={loadingChallenge} style={{ width: 54, padding: "8px 10px", borderRadius: 6, border: "none", fontSize: 14, fontWeight: 700, textAlign: "center", outline: "none" }} />
              </div>
            </div>
            <div style={{ display: "flex", gap: 8 }}>
              <button type="button" onClick={handleSubmit} disabled={status === "loading" || loadingChallenge} style={{ flex: 1, padding: "12px", borderRadius: 8, border: "none", background: COLORS.surface, color: COLORS.primary, fontFamily: FONTS.headline, fontWeight: 700, fontSize: 13, cursor: (status === "loading" || loadingChallenge) ? "not-allowed" : "pointer", opacity: (status === "loading" || loadingChallenge) ? 0.7 : 1 }}>{status === "loading" ? "..." : "Subscribe"}</button>
              <button type="button" onClick={() => { setShowCaptcha(false); setMathInput(""); setError(""); setChallenge(null); }} style={{ padding: "12px 14px", borderRadius: 8, border: "1px solid rgba(255,255,255,0.2)", background: "transparent", color: "rgba(255,255,255,0.7)", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>Back</button>
            </div>
            {error && <p style={{ color: "#fca5a5", fontSize: 11, fontWeight: 600, marginTop: 6 }}>{error}</p>}
          </div>
        )}
