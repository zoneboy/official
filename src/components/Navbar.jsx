...
export default function Navbar({ currentPage, setPage }) {
  ...
  // Highlight Gallery nav item when on either /gallery or /gallery/<slug>
  const isCurrentPage = (page) => {
    if (page === "gallery") return currentPage === "gallery" || currentPage === "galleryDetail";
    if (page === "blog") return currentPage === "blog" || currentPage === "article";
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", maxWidth: 1400, margin: "0 auto", padding: isMobile ? "14px 20px" : "16px 32px" }}>
        ...
        {!isMobile && (
          <div style={{ display: "flex", alignItems: "center", gap: 28 }}>
            {NAV_ITEMS.map((n) => {
              const active = isCurrentPage(n.page);
              return (
                <a key={n.page} href="#" onClick={(e) => { e.preventDefault(); navigate(n.page); }} style={{ fontFamily: FONTS.headline, fontWeight: 700, fontSize: 13, color: active ? "#15803d" : "#64748b", borderBottom: active ? "2px solid #15803d" : "2px solid transparent", paddingBottom: 4, textDecoration: "none" }}>{n.label}</a>
              );
            })}
      ...
      {isMobile && menuOpen && (
        <div style={{ background: COLORS.surfaceContainerLowest, borderTop: "1px solid rgba(0,0,0,0.05)", padding: "16px 20px 24px", display: "flex", flexDirection: "column", gap: 4 }}>
          {NAV_ITEMS.map((n) => {
            const active = isCurrentPage(n.page);
            return (
              <a key={n.page} href="#" onClick={(e) => { e.preventDefault(); navigate(n.page); }} style={{ fontFamily: FONTS.headline, fontWeight: 700, fontSize: 15, color: active ? COLORS.primary : COLORS.onSurfaceVariant, padding: "12px 8px", borderRadius: 8, background: active ? `${COLORS.primary}10` : "transparent", textDecoration: "none" }}>{n.label}</a>
            );
          })}
